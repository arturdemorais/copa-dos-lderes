...
export const feedbackSuggestionService = {
  ...
  async suggestLeader(fromLeaderId: string): Promise<string | null> {
    ...
    const recentLeaderIds = (recentSuggestions || []).map(
      (s) => s.suggested_leader_id
    );
    ...
    const recentEvaluationIds = (recentEvaluations || []).map(
      (e) => e.to_leader_id
    );
    ...
    const availableLeaders = allLeaders.filter(
      (l) =>
        !recentLeaderIds.includes(l.id) && !recentEvaluationIds.includes(l.id)
    );
    ...
    const sameTeamLeaders = availableLeaders.filter(
      (l) => l.team === fromLeader?.team
    );

    if (sameTeamLeaders.length > 0 && Math.random() < 0.5) {
      return sameTeamLeaders[Math.floor(Math.random() * sameTeamLeaders.length)]
        .id;
    }

    // Caso contrário, escolher aleatório
    return availableLeaders[Math.floor(Math.random() * availableLeaders.length)]
      .id;
  },
