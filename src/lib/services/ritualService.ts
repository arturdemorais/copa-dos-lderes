...
export const ritualService = {
   ...
   */
  async create(name: string, type: 'daily' | 'weekly' | 'rmr', date: string): Promise<Ritual> {
   ...
   */
  async markAttendance(ritualId: string, leaderId: string, present: boolean): Promise<void> {
   ...
   */
  async getLeaderAttendance(leaderId: string, startDate?: string): Promise<RitualAttendance[]> {
    let query = supabase
      .from('ritual_attendance')
      .select(`
        id,
   ...
   */
  async calculateAttendanceRate(leaderId: string, days: number = 30): Promise<number> {
    ...
    const { data: attendance, error: attendanceError } = await supabase
      ...
      .eq('leader_id', leaderId)
      .eq('present', true)
      .in('ritual_id', (rituals || []).map(r => r.id))
